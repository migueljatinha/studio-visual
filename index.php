...
{   
	...
	for (;$i < $n; $i++) {
		$ai = array_search($a[$i], $alphabet);
		$bi = array_search($b[$i], $alphabet);
		if($ai == $bi) continue;
		return $ai < $bi ? -1 : +1;
	}
...
$booglan_nice_numbers_arr = array_filter( $booglan_numbers_arr, function($word) use ($alphabet) {
	...
	{
		$v = array_search(substr($word, $i, 1), $alphabet);
		//var_dump(base_convert($v, 10, 20));
		$value += $v * ( ($i+1) * 20 );
	}
